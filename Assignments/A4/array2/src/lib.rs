...
impl<T: Clone> Array2<T> {
    ...
    pub fn flip_horizontal (&mut self, iter_row_major: bool) {
        let mut result = Array2::from_row_major( self.height, self.width, &self.array.clone()).unwrap();

        if iter_row_major {
            for elements in self.iter_row_major() {
                *result.get_mut(self.width - elements.0 - 1, elements.1).unwrap() = elements.2.clone();
            }
        } else {
            for elements in self.iter_col_major() {
                *result.get_mut(self.width - elements.0 - 1, elements.1).unwrap() = elements.2.clone();
            }
        }

        self.array = result.elements_row_major().clone();
    ...
    pub fn flip_vertical (&mut self, iter_row_major: bool) {
        ...
        if iter_row_major {
            for elements in self.iter_row_major() {
                *result.get_mut(elements.0, self.height - elements.1 - 1).unwrap() = elements.2.clone();
            }
        } else {
            for elements in self.iter_col_major() {
                *result.get_mut(elements.0, self.height - elements.1 - 1).unwrap() = elements.2.clone();
    ...
    pub fn rotate_90 (&mut self, iter_row_major: bool) {
        ...
        if iter_row_major {
            for elements in self.iter_row_major() {
                *result.get_mut(self.height - elements.1 - 1, elements.0).unwrap() = elements.2.clone();
            }
        } else {
            for elements in self.iter_col_major() {
                *result.get_mut(self.height - elements.1 - 1, elements.0).unwrap() = elements.2.clone();
    ...
    pub fn rotate_180 (&mut self, iter_row_major: bool) {
        let mut result = Array2::from_row_major(self.width, self.height, &self.array.clone()).unwrap();

        if iter_row_major {
            for elements in self.iter_row_major() {
                *result.get_mut(self.width - elements.0 - 1, self.height - elements.1 - 1).unwrap() = elements.2.clone();
            }
        } else {
            for elements in self.iter_col_major() {
                *result.get_mut(self.width - elements.0 - 1, self.height - elements.1 - 1).unwrap() = elements.2.clone();
    ...
    pub fn rotate_270 (&mut self, iter_row_major: bool) {
        ...
        if iter_row_major {
            for elements in self.iter_row_major() {
                *result.get_mut(elements.1, self.width - elements.0 - 1).unwrap() = elements.2.clone();
            }
        } else {
            for elements in self.iter_col_major() {
                *result.get_mut(elements.1, self.width - elements.0 - 1).unwrap() = elements.2.clone();
    ...
    pub fn transpose (&mut self, iter_row_major: bool) {
        ...
        if iter_row_major {
            for elements in self.iter_row_major() {
                *result.get_mut(elements.1, elements.0).unwrap() = elements.2.clone();
            }
        } else {
            for elements in self.iter_col_major() {
                *result.get_mut(elements.1, elements.0).unwrap() = elements.2.clone();
    ...
    pub fn filled_with(height: usize, width: usize, element: T) -> Self
    where
        T: Clone,
    {
...
pub fn from_column_major(elements: &[T], height: usize, width: usize) -> Result<Self, Error> 
    ...
    /// Returns an Array2.
    /// Returns an error if the dimensions don't match.
pub fn from_row_major(width: usize, height: usize, elements: &[T], ) -> Result<Self, Error> 
    where
    T: Clone,
{
    let total_len = height * width;
...
pub fn get(&self, column: usize, row: usize) -> Option<&T> {
    self.get_index(column, row).map(|index| &self.array[index])
}
...
pub fn get_mut(&mut self, column: usize, row: usize) -> Option<&mut T> {
    self.get_index(column, row).map(move |index| &mut self.array[index])
}
...
#[allow(dead_code)]
pub fn set(&mut self, column: usize, row: usize, element: T) -> Result<(), Error> {
    self.get_mut(column, row).map(|location| {*location = element;}).ok_or(Error::IndicesOutOfBounds(row, column))
}
